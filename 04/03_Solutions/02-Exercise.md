...
do #??
    echo "Deploy app-${i}" #??
    kubectl create deployment app-${i} --image=gcr.io/google-samples/kubernetes-bootcamp:v1  #?? 
done #??
```
...
for pod in "${pods[@]}" #??
do #??
    curl http://localhost:8001/api/v1/namespaces/default/pods/$pod:8080/proxy/ #?? 
done #??
```
